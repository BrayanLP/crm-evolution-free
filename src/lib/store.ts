...
export function useLeads() {
  ...
  const getHistory = async (remoteJid: string): Promise<ChatMessage[]> => {
    if (!historyWebhookUrl) return [];
    try {
      // Usamos GET pasando el remoteJid como parámetro
      ...
      if (response.ok) {
        const data = await response.json();
        // Mapeamos al formato esperado por el chat
  ...
  const updateSettings = (url: string, historyUrl: string, inst: string) => {
    setWebhookUrl(url);
